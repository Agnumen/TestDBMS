...
from app.infrastructure.database.dao.database import async_session_maker
            ...
            async with async_session_maker() as session:
                try:
                    if isolation_level:
                        ...
                        await session.execute(stmt, {"level": isolation_level})
...
def connection_cl(self, isolation_level: Optional[str] = None, commit: bool = True):
    """
    Декоратор для управления сессией с возможностью настройки уровня изоляции и коммита.
    Параметры:
    - `isolation_level`: уровень изоляции для транзакции (например, "SERIALIZABLE").
